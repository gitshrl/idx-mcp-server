...
## Round 4 — the code added since (2026-05-31)

Rounds 1-3 hardened the `run_query` engine. This round targets the code that landed afterward — the OAuth 2.1 AS, the `get_filing` egress tool + its cache, the usage logger, the dual API-key/OAuth auth, and the recent analytics/etl changes — reviewed by a multi-agent adversarial pass (6 dimension finders → every finding verified by two independent lenses: a refuter and a code-tracer). **16 findings: 13 confirmed, 2 disputed, 1 rejected.** The 2-lens verify earned its keep: it rejected a confident-but-wrong "wreq follows redirects to any host" finding (the finder misread wreq's default as `Policy::limited(10)`; `ClientBuilder` actually defaults to `Policy::none()`).

### Fixed (commit 3fa2ad1)

- **SSRF — `validate_url` host-confusion (HIGH).** The hand-rolled `strip_prefix("https://").split('/')` host parse disagreed with the WHATWG `url` crate that `wreq` actually uses: `https://evil.com\.idx.co.id/` passed the allowlist (`.ends_with(".idx.co.id")`) but the client resolves host `evil.com`. Full egress-allowlist bypass on the one tool allowed to egress (cloud metadata, internal hosts). **Fix:** parse with `url::Url`, require `https`, reject credentials and IP-literal hosts, match the parsed host — so validator and client can't disagree. Added bypass tests (backslash, userinfo, suffix, IP, case).
- **Non-finite ratio → invalid JSON (HIGH, a regression from the round-3-era `TRY_CAST` fix).** `INRU.trailing_pe` is literally `"Infinity"` in `summary`; `TRY_CAST(... AS DOUBLE)` yields `inf`, and `to_json(inf)` emits bare `Infinity` (invalid JSON) — hard-failing any `screen_stocks` / `SELECT * FROM latest` whose result set includes that ticker. **Fix:** `CASE WHEN isfinite(TRY_CAST(...)) THEN TRY_CAST(...) END` on every ratio column in the `latest` view and in `get_company`. Verified live end-to-end (`run_query` returns `trailing_pe: null`, no error).
- **`temp_directory` PRAGMA built from an unescaped path (LOW).** A single quote in `TMPDIR`/`IDX_SERVING_DIR` would break the boot/ETL PRAGMA. **Fix:** escape with `.replace('\'', "''")` (mirrors the SQL validator), in both `analytics.rs` and `etl.rs`.
- **Bearer scheme matched case-sensitively (LOW).** `strip_prefix("Bearer ")` rejected a spec-valid lowercase scheme. **Fix:** split on the first space, compare scheme with `eq_ignore_ascii_case`.

### Deferred — design decision / larger work (NOT silently dropped)

- **OAuth is effectively open (HIGH-by-impact, by-design MVP).** `/oauth/register` is unauthenticated + uncapped and `/authorize` auto-consents with no user login, so any caller that can reach the server mints a 30-day `/mcp` token. Acceptable only for a closed/loopback deployment; the current public tunnel exposes it. **Mitigation pending the monetization milestone** (real accounts/consent, gated + rate-limited DCR, prune job for `oauth_clients`/codes/tokens). Until then, treat the public URL as a demo, not a protected resource.
- **`valid_redirect` prefix-matching (MEDIUM, latent).** Same WHATWG class of bug as the SSRF one (`http://localhost@evil.com`, bare `https://`, suffixes pass). Latent today because auto-consent means there's no victim session to steal a code from; becomes directly exploitable the moment real user auth lands. Fix with the same `url`-crate parse when gating OAuth.
- **`get_filing` concurrency/leaks (MEDIUM).** (1) the browser `Session` `tokio::Mutex` is held across the ~25-30s clearance+retry+relaunch, serializing every fallback fetch; (2) a dropped/relaunched `Browser` leaks chrome child processes (observed operationally) — needs explicit `close()/kill()` teardown instead of relying on `Drop`; (3) the L1 in-memory cache (and the L2 SQLite table) are unbounded — needs an LRU/byte cap + retention prune.
- **RFC 8707 `resource` accepted unvalidated (LOW).** Client-supplied `resource` is bound verbatim into the token audience with no `invalid_target` check. Not exploitable (opaque tokens, no introspection, canonical-audience-only verification) but spec-incorrect and can mint silently-dead tokens; validate against `st.resource()` when gating OAuth.
- **`get_filing` error classification + unbounded body read (LOW).** Client-input failures (off-host URL, body-not-PDF) surface as a generic `internal_error` instead of `invalid_params`; the fetched body is buffered fully into RAM (bounded only by host-allowlist + 30s timeout) with no max-bytes cap.

Verified: `clippy` pedantic `-D warnings` + `fmt` clean, **15 unit tests + `engine_on_real_data` pass**, and the two HIGH fixes confirmed live over MCP on the running server.
