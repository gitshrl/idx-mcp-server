...
## Round 6 — deep read of the current tree (2026-05-30, HEAD `790c6a2`)

Full-file read of the post-feature code (`src/` now ~3,340 LOC across analytics, auth, catalog, config, filings, keys, main, oauth, server). This pass reads each module whole rather than diffing, so it catches things a diff hides. It also verifies the three latest fixes (`f7be56f` bounded usage writer + retention, `790c6a2` DuckDB temp-dir + `TRY_CAST`, `6f055a6` docs sync).

### Verified fixes — all correct

- **Bounded usage writer (`keys.rs` `UsageLogger`)** — this is the right design and cleanly executed. One background task owns the DB writes; the request path calls `record()` → `tx.try_send()` (non-blocking, **drops on a full 4096-slot queue** — correct for best-effort telemetry, no backpressure onto user requests). The writer drains with `recv_many` (up to 128) and **batch-inserts in a single transaction**; `recv_many` returning 0 = all senders dropped = clean loop exit. A `tokio::select!` arm prunes rows older than 90 days every 6h with `MissedTickBehavior::Skip`. This fully closes the round-3/5 "unbounded growth + unbounded per-request spawn" finding. The earlier hot-path SQLite contention is gone too.
- **DuckDB temp spill (`analytics.rs` `build_serving`)** — `SET temp_directory` now points at the per-instance serving dir (reaped on restart), so large builds spill there instead of cwd. Good.
- **`TRY_CAST(... AS DOUBLE)` on the summary ratios in `latest`** — real correctness fix: Yahoo ratio columns can arrive as strings/mixed, and the old plain projection could later break `screen_stocks`' `CAST(? AS DOUBLE)` comparisons or error the view. `TRY_CAST` yields NULL instead of failing. Defensive and right.

Net of the three: the operational and correctness posture is back up. **But none of them touched the security hole from the feature round**, so the headline number doesn't move.

### Still open — the showstopper hasn't been touched

**🔴 OAuth `/authorize` auto-consents — full auth bypass (`oauth.rs:7`, `:124`, `:153`).** Re-read in full and confirmed unchanged: there is no login, no consent, no user identity anywhere in the authorization-code flow. Any caller that can reach the server runs register → authorize → token and gets a valid, audience-bound, **30-day** opaque token (`TOKEN_TTL_SECS = 30*24*3600`) that satisfies `auth_middleware` for every MCP tool — the API-key gate is bypassed entirely via this path. The OAuth *mechanics* are otherwise sound (PKCE S256 verified; codes single-use via atomic `UPDATE … WHERE used=0 … RETURNING` with 60s TTL; `redirect_uri` checked against the registered set before the redirect is trusted; codes/tokens stored only as SHA-256). The defect is the missing authorization decision, not the crypto. For an internet-facing server this is an open door and must be gated (real consent/login, or a hard network boundary) before public exposure. The module's own docstring admits it ("anyone who can reach the server can obtain a token") — that honesty is good, but the comment is not a mitigation.

**🟠 `get_filing` SSRF guard is hand-rolled (`filings.rs` `validate_url`).** `strip_prefix("https://")` then `split('/').next()` as the host check. It happens to reject `https://idx.co.id@evil.com/...`, but a security boundary shouldn't rest on "happens to" — parse with the `url` crate and compare the parsed host. Plus: headless Chrome launches `--no-sandbox` while fetching attacker-influenceable URLs, and per-launch Chrome profile dirs (`idx-mcp-chrome-{pid}-{n}`) are never cleaned up (the round-3 temp-dir leak, reintroduced for the browser path). `pdf-extract` loads the whole body and iterates `chars().count()` over the full text — a large PDF can spike memory.

### New observations from reading the whole tree

- **`http://localhost` / `http://127.0.0.1` redirect URIs are accepted (`oauth.rs:263` `valid_redirect`).** Reasonable for native MCP clients, but combined with open registration + auto-consent it widens the surface: anyone can register a localhost redirect and complete the flow. Belongs in the same gating decision as the auto-consent fix.
- **OAuth audience defaults silently (`oauth.rs:234`).** `consumed.resource.unwrap_or_else(|| st.resource())` — if a client omits `resource`, the token is minted for the server's own `/mcp` audience. Functional, but means RFC 8707 audience-restriction is opt-in by the client rather than enforced.
- **`public_url` is plaintext HTTP by default (`config.rs`)** — `http://{bind_addr}`. The token audience is byte-compared, so a prod deploy that forgets `IDX_PUBLIC_URL` silently issues OAuth-over-HTTP bound to `http://127.0.0.1:8080`. The server should refuse to start with a non-HTTPS, non-localhost `public_url`.
- **Tool error mapping is inconsistent (`server.rs`).** `get_filing` routes failures through `mcp_err` (now a generic "internal error" + logged) — good — but that means a bad-but-user-caused filing URL (off-host, not a PDF) comes back as an opaque internal error rather than an `invalid_params` the agent can act on. `run_query`/`screen_stocks` correctly use `user_err`. `get_filing` should split user errors (off-host, non-PDF) from backend failures.
- **`get_filing` has no per-call deadline at the tool layer.** The wreq path has a 30s timeout, but the browser fallback can run 4×3s retries + a relaunch + navigation waits, so a single call can pin a request well past 30s with no overall cap.
- **`usage.rows` is still hard-wired to 0 (`keys.rs` `log_usage_batch` inserts the literal `0`).** Harmless (telemetry not yet wired to real row counts), but the column remains decorative — note it so it isn't mistaken for real data.

### Net

The three latest commits are genuinely good engineering and resolve the operational/correctness items precisely. But a deep read confirms the **core security regression from the feature round is still wide open**: an internet-facing service that hands a 30-day token to anyone who asks. Until the OAuth flow gains a real authorization decision (and the SSRF parser is replaced + Chrome profile dirs cleaned), the effective score stays around **7/10** — the craftsmanship is high, but "anyone can mint a token" caps it regardless of how clean everything around it is. Highest-value next step is unchanged and singular: **gate `/authorize`.**
